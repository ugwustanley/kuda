...
import "./SidebarMechanic.scss"

const SidebarMechanic = ({content  , current , showStaff}) => {
    const [weekly , setWeekly] = useState(false)
    const [tab , setTab] = useState("")
    const [profile , setProfile] = useState(false)
    const [approve , setApprove] = useState(false)
    const [todaytab , setTodaytab] = useState(["New Mechanics" , "Ongoing repairs" , "5 star mechanics" , "New bookings"])
    ...
    const heading = ["Total Bookings" , "Ongoing Repairs" , "Repairs Completed" , "New Bookings"]
        ...
        <React.Fragment>
            <div className={profile == true || approve === true ? `admindash__main fit_height`: `admindash__main`}>
                         <div className="admindash__overview">
                               {
                                   data.map( (one ,index) => <div className="total__orders">
                                            <p>{heading[index]}</p>
                                            <h4>{one.total}</h4>
                                   </div>)
                               }
                         ...
                         <div className="current__event">
                             {/* ["Total Bookings" , "Ongoing Repairs" , "Repairs Completed" , "New Bookings"] */}
